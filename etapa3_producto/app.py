...
from .demo import DemoBackend, normalize
...
def create_app(backend: Backend | None = None, *, mode: str | None = None, ttl: float = 3600, capacity: int = 100) -> FastAPI:
    ...
    def chat(request: ChatRequest):
        # Serializa turnos: evita escrituras perdidas. Prototipo local de un worker.
        with lock:
            prune()
            ...
            try:
                ...
                if selected_mode in {"groq", "live"} and any(f"[{key}]" not in generation.answer for key in generation.cited_chunk_ids):
