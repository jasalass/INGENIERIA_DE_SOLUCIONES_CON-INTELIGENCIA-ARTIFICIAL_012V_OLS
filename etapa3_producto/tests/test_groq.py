...
class GroqTests(unittest.TestCase):
    ...
    def test_article_and_followup_use_retrieved_evidence(self):
        ...
        self.outputs.extend([answer(f"Extracto [{source.chunk_id}]", [source.chunk_id]), plan("Excepciones del artículo 4", "document"), answer(f"Consulta [{source.chunk_id}]", [source.chunk_id])])
    ...
    def test_explicit_nonexistent_article_needs_no_model_call(self):
        client = self.make_client([])
    ...
    def test_citation_must_also_be_visible(self):
        client = self.make_client([])
        source = self.backend.retrieve("artículo 4", 1, Filters())[0]
        self.outputs.append(answer("Sin cita visible", [source.chunk_id]))
        self.assertEqual(client.post("/chat", json={"message": "artículo 4"}).status_code, 503)
