...
from .demo import DemoBackend, parse_article_reference
...
class GroqBackend(DemoBackend):
    ...
    def plan_query(self, question: str, history: list[Turn]) -> QueryPlan:
        plan = QueryPlan.model_validate(self._invoke(PLAN_PROMPT, question, history))
        article, _ = parse_article_reference(question)
        # El clasificador (con historial) decide document vs conversation --ya
        # no se salta con un regex, porque eso podia clasificar como "document"
        # un mensaje que solo menciona un articulo de pasada (ej. "ya me
        # explicaste el articulo 4, ahora dime como me llamaba"). Lo unico
        # que se fuerza es que, si es document, la reformulacion del LLM no
        # haya perdido el numero de articulo explicito.
        if article and plan.kind == "document" and article not in plan.query:
            plan = QueryPlan(kind="document", query=question)
        return plan

    def retrieve(self, query: str, top_k: int, filters: Filters) -> list[Source]:
        sources = super().retrieve(query, top_k, filters)
        article, _ = parse_article_reference(query)
        return with_document_context(sources, self.document_context, filters,
                                     explicit_article=bool(article))
